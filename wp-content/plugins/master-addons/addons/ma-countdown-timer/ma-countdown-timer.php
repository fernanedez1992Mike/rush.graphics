	...
	protected function render()
	{
		...
		foreach ($date_attr as $attr => $key) {
			...
			if ($key['display']) {
				...
				$attr_markup .= '<span class="jltma-countdown-value jltma-countdown-' . esc_attr($attr) . '">' . __('0', JLTMA_TD) . '</span>';
		...
		$this->add_render_attribute(
			...
			[
				'class'    => [
					...
					'jltma-countdown',
					'jltma-countdown-' . esc_attr($settings['ma_el_countdown_style'])
				],
		...
		$extra_classes = 'jltma-countdown-' . esc_attr($countdown_style) . ' ';

		$output = '<section class="widget-container jltma-widget-countdown"><div class="jltma-countdown-wrapper ' . esc_attr($extra_classes) . '"' . esc_attr($data_attr) . '>';
		$output .= $attr_markup;
		$output .= '</div></div>';
