		...
		if (is_array($settings['jltma_counterup_contents'])) :
			...
			foreach ($settings['jltma_counterup_contents'] as $index => $list) :
				...
				if (!empty($list['number']) || ($list['title'])) :
					...
					$list['number'] ? printf('<div class="jltma-counter-up-number-section">%1$s<h3 class="jltma-counter-up-number">%2$s</h3>%3$s</div>', $this->parse_text_editor($list['number_prefix']), esc_attr($list['number']), $this->parse_text_editor($list['number_suffix'])) : '';
