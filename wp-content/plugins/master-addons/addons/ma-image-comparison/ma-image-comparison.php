...
class JLTMA_Image_Comparison extends Widget_Base
{
	...
	{
		return esc_html__('Image Comparison', JLTMA_TD);
	}
	...
	protected function register_controls()
	{
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__('Images', JLTMA_TD)
			]
		);
		...
		$this->start_controls_tab(
			...
			[
				'label' => esc_html__('Before', JLTMA_TD),
			]
		);
		...
		$this->add_control(
			...
			array(
				'label'      => esc_html__('Before image', JLTMA_TD),
		...
		$this->add_control(
			...
			[
				...
				'default'     => esc_html__('Before', JLTMA_TD),
				'placeholder' => esc_html__('Before Label', JLTMA_TD),
				'description' => esc_html__('Show/Hide Labels on Overlay Settings', JLTMA_TD),
				'dynamic'     => [
					'active' => true,
				]
			]
		...
		$this->start_controls_tab(
			...
			[
				'label' => esc_html__('After', JLTMA_TD),
			]
		);
		...
		$this->add_control(
			...
			array(
				'label'      => esc_html__('After Image', JLTMA_TD),
				'type'       => Controls_Manager::MEDIA,
				'show_label' => false
			)
		);
		...
		$this->add_control(
			...
			[
				'label'       => esc_html__('Label', JLTMA_TD),
				'type'        => Controls_Manager::TEXT,
				'default'     => esc_html__('After', JLTMA_TD),
				'placeholder' => esc_html__('After Label', JLTMA_TD),
				'description' => esc_html__('Show/Hide Labels on Overlay Settings', JLTMA_TD),
				'dynamic'     => [
					'active' => true,
				]
		...
		$this->start_controls_section(
			...
			array(
				'label' => esc_html__('Settings', JLTMA_TD)
			)
		);
		...
		$this->add_control(
			...
			[
				'label'      => esc_html__('Visible Ratio (%)', JLTMA_TD),
		...
		$this->add_control(
			...
			[
				'label'       => esc_html__('Orientation', JLTMA_TD),
				'type'        => Controls_Manager::CHOOSE,
				'label_block' => false,
				'options'     => [
					'horizontal' 	=> [
						'title' => esc_html__('Horizontal', JLTMA_TD),
						'icon'  => 'fa fa-arrows-h',
					],
					'vertical' 		=> [
		...
		$this->add_control(
			...
			[
				'label'          => esc_html__('Overlay', JLTMA_TD),
				'type'           => Controls_Manager::SWITCHER,
				'default'        => 'yes',
				'label_on'       => esc_html__('Show', JLTMA_TD),
				'label_off'      => esc_html__('Hide', JLTMA_TD),
				'return_value'   => 'yes',
				'description'    => esc_html__('Show/Hide overlay with before and after label', JLTMA_TD),
				'style_transfer' => true,
			]
		);
		...
		if (ma_el_fs()->can_use_premium_code()) {
			$this->add_control(
				...
				[
					'label'   => esc_html__('Move Handle', JLTMA_TD),
					'type'    => Controls_Manager::SELECT,
					'default' => 'drag',
					'options' => [
						'drag'        => esc_html__('Mouse Drag/Swipe', JLTMA_TD),
						'mouse_move'  => esc_html__('Mouse Move', JLTMA_TD),
						'mouse_click' => esc_html__('Mouse Click', JLTMA_TD),
					],
					'style_transfer' => true,
				]
			);
			...
			$this->add_control(
				...
				[
					...
					'options' => [
						'drag'   => esc_html__('Mouse Drag/Swipe', JLTMA_TD),
						'mm_pro' => esc_html__('Mouse Move (Pro)', JLTMA_TD),
						'mc_pro' => esc_html__('Mouse Click (Pro)', JLTMA_TD),
					],
					'style_transfer' => true,
				]
			);
		...
		$this->start_controls_section(
			...
			array(
				'label'      => esc_html__('Layout Style', JLTMA_TD),
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__('Handle', JLTMA_TD),
				'tab'   => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__('Handle Bar', JLTMA_TD),
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__('Handle Style', JLTMA_TD),
				'type'      => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				'label'      => esc_html__('Box Width', JLTMA_TD),
				'type'       => Controls_Manager::SLIDER,
				'size_units' => ['px'],
				'range'      => [
					'px' => [
		...
		$this->add_responsive_control(
			...
			[
				'label'      => esc_html__('Box Height', JLTMA_TD),
				'type'       => Controls_Manager::SLIDER,
				'size_units' => ['px'],
				'range'      => [
					'px' => [
		...
		$this->add_control(
			...
			[
				...
				'raw'             => sprintf(esc_html__('%1$s Live Demo %2$s', JLTMA_TD), '<a href="https://master-addons.com/demos/image-comparison/" target="_blank" rel="noopener">', '</a>'),
		...
		$this->add_control(
			...
			[
				'type'            => Controls_Manager::RAW_HTML,
				'raw'             => sprintf(esc_html__('%1$s Documentation %2$s', JLTMA_TD), '<a href="https://master-addons.com/docs/addons/image-comparison-element/?utm_source=widget&utm_medium=panel&utm_campaign=dashboard" target="_blank" rel="noopener">', '</a>'),
				'content_classes' => 'jltma-editor-doc-links',
			]
		);
		...
		$this->add_control(
			...
			[
				'type'            => Controls_Manager::RAW_HTML,
				'raw'             => sprintf(esc_html__('%1$s Watch Video Tutorial %2$s', JLTMA_TD), '<a href="https://www.youtube.com/watch?v=3nqRRXSGk3M" target="_blank" rel="noopener">', '</a>'),
				'content_classes' => 'jltma-editor-doc-links',
			]
		);
		$this->end_controls_section();
