	...
	protected function render()
	{
		...
		if (count($settings['ma_el_image_gallery_items']) > 1) {
			...
			if (is_array($settings['ma_el_image_gallery_items'])) :
				...
				jltma_print_unescaped_internal_string('<div ' . $ma_el_image_filter_gallery_editor . '>');
				...
				foreach ($settings['ma_el_image_gallery_items'] as $index => $item) :
					...
					if (!empty($images)) {
						foreach ($images as $image) {
							...
							if (!empty($image['id'])) {
								$images = wp_get_attachment_image($image['id'], esc_attr($settings['ma_el_image_gallery_image_size']));
								echo $images;
							} else {
