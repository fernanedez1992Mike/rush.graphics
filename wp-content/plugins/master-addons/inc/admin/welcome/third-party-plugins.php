...
<h3><?php echo esc_html__('Third Party Plugins', JLTMA_TD); ?></h3>
						...
						if ($plugin_slug and $plugin_file) {
							if (Master_Addons_Helper::is_plugin_installed($plugin_slug, $plugin_file)) {
								...
								if (!jltma_is_plugin_active($plugin_file)) {
									...
									$html = '<a class="jltma-external-plugin-download jltma-external-plugin-download-active" href="' . $activation_url . '" ><span class="jltma-external-plugin-download jltma-external-plugin-download-active pr-1">' . esc_html__('Activate', JLTMA_TD) . '</span><i class="dashicons dashicons-yes-alt"></i></a>';
								} else {
									$html = '';
								}
							} else {
								...
								$html = '<a class="jltma-external-plugin-download" href="' . $install_url . '"><span class="jltma-external-plugin-download-text">' . esc_html__('Download', JLTMA_TD) . '</span><i class="dashicons dashicons-download"></i></a>';

								activate_plugin($plugin_file);
							}
							echo jltma_print_unescaped_internal_string($html);
						}
						?>
					</div>
