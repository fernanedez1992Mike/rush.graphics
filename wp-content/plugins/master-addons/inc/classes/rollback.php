...
class Master_Addons_Rollback
{
	...
	public function jltma_post_addons_rollback()
	{
		...
		if (empty($_GET['version']) || !in_array($_GET['version'], $rollback_versions)) {
			wp_die(esc_html__('Error occurred, The version selected is invalid. Try selecting different version.', JLTMA_TD));
		}
		...
		wp_die('', __('Rollback to Previous Version', JLTMA_TD), ['response' => 200,]);
	}
...
<?php
	...
	protected function upgrade()
	{
		...
		$upgrader_args = [
			...
			'title' => '<img src="' . esc_url($logo_url) . '" alt="Master Addons Version Rollback">' . __('Rollback to Previous Version ', JLTMA_TD),
		];
