...
function jltma_dashboard_widgets()
{
	...
	wp_add_dashboard_widget(
		...
		'<img src="' . JLTMA_URL . '/assets/images/icon.png' . '" height="20" width="20">' .
			esc_html__('Master Addons News & Updates', JLTMA_TD),
...
function get_dashboard_overview_widget_footer_actions()
{
	$base_actions = [
		...
		'help' => [
			'title' => esc_html__('Help', JLTMA_TD),
	...
	$additions_actions = [
		'go-pro' => [
			'title' => esc_html__('Go Pro', JLTMA_TD),
...
function jltma_dashboard_news_feed()
{
	...
	wp_widget_rss_output(array(
		...
		'title' 		=> esc_html__('Master Addons News & Updates', JLTMA_TD),
		'items' 		=> 5,
		'show_summary' 	=> 0,
		'show_author' 	=> 0,
		'show_date' 	=> 0
		...
		<ul>
			<?php foreach (get_dashboard_overview_widget_footer_actions() as $action_id => $action) : ?>
				<li class="ma-el-overview__<?php echo esc_attr($action_id); ?>"><a href="<?php echo esc_attr(
																								$action['link']
																							); ?>" target="_blank"><?php echo esc_html($action['title']); ?> <span class="screen-reader-text"><?php echo __('(opens in a new window)', JLTMA_TD);
...
function jltma_print_unescaped_internal_string($string)
{
