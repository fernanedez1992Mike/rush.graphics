	...
	protected function register_controls()
	{
		...
		if (ma_el_fs()->can_use_premium_code()) {
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('First configure API Settings <a href="%1$s">click here</a>', JLTMA_TD), admin_url('admin.php?page=master-addons-settings#ma_api_keys')),
		...
		if (ma_el_fs()->can_use_premium_code()) {
			...
			$this->add_control(
				...
				[
					...
					'fields'          => [
						...
						// 	'label_block'   => true,
						// 	'description'   => esc_html__( 'One Options per line', JLTMA_TD ),
						//                 'condition'     => [
						//                     'field_type'       => 'checkbox'
						//                 ]
						// ],
			...
			$this->add_responsive_control(
				...
				[
					'label'   => __('Comment Extra Fields', JLTMA_TD),
					'type'    => Controls_Manager::CHOOSE,
					'options' => [
						'1' => [
							'title' => esc_html__('', JLTMA_TD),
	...
	public function jltma_frontend_form_build()
	{
		...
		if (comments_open()) {
			...
			if (is_user_logged_in() && JLTMA_Comments_Builder::jltma_comment_elementor_preview_mode()) {
				echo esc_html__('Check Frontend Comment Form for reCaptcha', JLTMA_TD);
			}
			...
			$comments_args = array(
				...
				'logged_in_as' => '<p class="logged-in-as">' . sprintf(esc_html__('Logged in as %1$s. %2$sLog out &raquo;%3$s', JLTMA_TD), '<a href="' . esc_url(admin_url('profile.php')) . '">' . esc_html($user_identity) . '</a>', '<a href="' . wp_logout_url(apply_filters('the_permalink', get_permalink())) . '" title="' . esc_html__('Log out of this account', JLTMA_TD) . '">', '</a>') . '</p>',
